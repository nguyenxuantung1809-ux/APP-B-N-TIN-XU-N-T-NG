...
export function BulletinPreview({ data, editorMode, layout, selectedTargetIds, selectedImageId, onSelectTarget, onSelectImage, onClearSelection, onLayoutChange, onBeginChange }: BulletinPreviewProps) {
  ...
  const blocks: Record<BulletinBlockId, React.ReactNode> = {
    ...
    marketLegend: <RootFrame id="marketLegend" label={data.labels.chart} icon={renderEditableImage('sectionIcon:chart', 'Chart Section Icon', { defaultNode: <ChartNoAxesCombined size={21} />, className: 'root-section-icon', sizes: '62px' })} isEditing={isEditing} selected={isRootSelected('marketLegend')} onSelect={(additive) => selectRoot('marketLegend', data.labels.chart, additive)} style={layout.styles.marketLegend} className="market-chart-block"><UsdVndChart data={data.usdVndChart} locale={data.locale} /></RootFrame>,
